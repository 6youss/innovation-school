...
class SessionDetails extends Component {
    ...
    componentDidMount(){
        fetch("http://localhost:3001/session/"+this.props.match.params.id)
        .then( res => res.json())
        .then(json=>{
    ...
    getSessionStudents(sessionId,groupId,sessionDone){
        if(sessionDone){
            const students=fetch("http://localhost:3001/session/"+sessionId+"/students")
            .then( res => res.json());
            
            const studentsReviews = fetch("http://localhost:3001/session/"+sessionId+"/reviews")
            .then( res => res.json());
            ...
            fetch("http://localhost:3001/group/"+groupId+"/students")
            .then( res => res.json())
            .then(json=>{
                let presentStudents = {};
    ...
    markAbsents(){
        ...
        this.state.students.forEach(student => {
            if( (this.state.absentStudents.indexOf(student.studentId) !== -1) )
                markStudents.push(
                    fetch("http://localhost:3001/session/"+this.state.session.sessionId+"/absent",{
                        method: "POST",        
                        headers:{
                ...
                markStudents.push(
                    fetch("http://localhost:3001/session/"+this.state.session.sessionId+"/present",{
                        method: "POST",        
                        headers:{
                            'Content-Type': 'application/json'
                        },
